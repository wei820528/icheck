...
cd C:\Program Files (x86)\Microsoft\Edge\Application
 if %HOUR% LEQ  %MaxDate% (
     if %HOUR% EQU  %TaskMax% (
        msedge.exe  https://localhost:447/iCheckAPI/Task/Task
		ping -n 10 127.1 >nul 3>nul
     ) 
     if %HOUR% LEQ  %NewSendMail% (
		msedge.exe  https://localhost:447/iCheckAPI/Task/NewSendMail    
		ping -n 10 127.1 >nul 3>nul 
	) 
	 if %HOUR% LEQ  %SendSystem% (
		msedge.exe  https://localhost:447/iCheckAPI/Task/SendSystem
		ping -n 10 127.1 >nul 3>nul
	 ) 
 ) 
